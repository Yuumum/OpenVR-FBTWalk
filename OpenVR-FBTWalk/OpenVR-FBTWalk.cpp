    ...
    if (Pcha.cha > _PLAYMOD.limit) {
        ...
        if (Pcha.ready.pointC) {
            ...
            if (Pcha.distance_ima - Pcha.distance_mae < 0) {
                orig.m[0][3] = orig.m[0][3] + (nowpose.m[0][3] - lastpose.m[0][3]) * (_PLAYMOD.speed / 100);
                orig.m[2][3] = orig.m[2][3] + (nowpose.m[2][3] - lastpose.m[2][3]) * (_PLAYMOD.speed / 100);
                newpose = orig;
        ...
        if (_PLAYMOD._POINTC.ready) {
            ...
            if (Pcha.distance_ima - Pcha.distance_mae < 0) {
                ...
                if (_PLAYMOD.smooth) {
                    ...
                    Pcha.s_i = Pcha.s_i + 1;
                    orig.m[0][3] = orig.m[0][3] + (Pcha.s_x / Pcha.s_i) * (_PLAYMOD.speed / 100);
                    orig.m[2][3] = orig.m[2][3] + (Pcha.s_z / Pcha.s_i) * (_PLAYMOD.speed / 100);
                }
                else
                {
                    orig.m[0][3] = orig.m[0][3] + (nowpose.m[0][3] - lastpose.m[0][3]) * (_PLAYMOD.speed / 100);
